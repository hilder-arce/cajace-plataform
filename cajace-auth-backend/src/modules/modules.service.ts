...
export class ModulesService {
  ...
  async create(createModuleDto: CreateModuleDto): Promise<Module> {
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException(`Conflicto: El módulo con el nombre "${createModuleDto.nombre}" ya existe en el sistema`);
      }
      throw error;
    }
  }
  ...
  async findAll(): Promise<Module[]> {
    return await this.moduleModel.find({ estado: true })
      .select('-__v')
      .exec();
  }
  ...
  async findOne(id: string): Promise<Module> {
    ...
    if (!module) throw new NotFoundException(`Módulo con ID "${id}" no encontrado`);
    if (!module.estado) throw new NotFoundException(`El módulo solicitado se encuentra inactivo`);
  ...
  async update(id: string, updateModuleDto: UpdateModuleDto): Promise<Module> {
    ...
    return await this.moduleModel.findByIdAndUpdate(id, updateModuleDto, { returnDocument: 'after' })
      .select('-__v')
      .exec() as Module;
  }
  ...
  async remove(id: string): Promise<Module> {
    ...
    return await this.moduleModel.findByIdAndUpdate(id, { estado: false }, { returnDocument: 'after' })
      .select('-__v')
      .exec() as Module;
  }
  ...
  async findAllInactive(): Promise<Module[]> {
    return await this.moduleModel.find({ estado: false })
      .select('-__v')
      .exec();
  }
  ...
  async findOneInactive(id: string): Promise<Module> {
    ...
    if (module.estado) throw new ConflictException(`El módulo consultado ya está activo`);
