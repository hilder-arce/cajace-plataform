...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto, creadoPor: string): Promise<User> {
    if (createUserDto.email) {
      ...
      if (existingUser) {
        throw new ConflictException(`El correo "${createUserDto.email}" ya se encuentra registrado`);
      }
    }
    ...
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException('Conflicto de duplicidad: El correo ya existe en el sistema');
      }
      throw error;
    }
  }
  ...
  async findOne(id: string): Promise<User> {
    ...
    if (!user) throw new NotFoundException(`Usuario activo con ID "${id}" no encontrado`);
    return user;
  }
  ...
  async findOneInactive(id: string): Promise<User> {
    ...
    if (!user) throw new NotFoundException(`Usuario inactivo con ID "${id}" no encontrado`);
    return user;
  }

  async update(id: string, updateUserDto: UpdateUserDto, actualizadoPor?: string): Promise<User> {
  ...
  async changePassword(id: string, dto: ChangePasswordDto): Promise<{ mensaje: string }> {
    const user = await this.userModel.findOne({ _id: id, estado: true }).select('+password').exec();

    if (!user) throw new NotFoundException('Usuario no encontrado para actualización de seguridad');

    const isMatch = await bcrypt.compare(dto.passwordActual, user.password);
    if (!isMatch) throw new BadRequestException('La validación de identidad falló: Contraseña actual incorrecta');
  ...
  async adminChangePassword(id: string, dto: AdminChangePasswordDto): Promise<{ mensaje: string }> {
    ...
    return { mensaje: 'Reseteo de contraseña administrativa completado exitosamente' };
  }
  async updateOwnProfile(id: string, updateUserDto: UpdateUserDto): Promise<User> {
