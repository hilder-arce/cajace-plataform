...
export class PermissionsService {
  ...
  // ======================================================
  async create(createPermissionDto: CreatePermissionDto, creadoPor: any): Promise<Permission> {
    ...
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException(`Conflicto: Ya existe un permiso con nombre "${createPermissionDto.nombre}" en el módulo "${module.nombre}"`);
      }
      throw error;
    }
  }
  ...
  async findAll(): Promise<Permission[]> {
    return await this.permissionModel.find({ estado: true })
      .select('-__v')
      .populate('modulo', 'nombre -_id')
      .exec();
  }
  ...
  async findOne(id: string): Promise<Permission> {
    ...
    if (!permission.estado) throw new NotFoundException(`El permiso solicitado se encuentra inactivo`);
  ...
  async update(id: string, updatePermissionDto: UpdatePermissionDto): Promise<Permission> {
    ...
    return await this.permissionModel.findByIdAndUpdate(id, updatePermissionDto, { returnDocument: 'after' })
      .select('-__v')
      .exec() as Permission;
  }
  ...
  async remove(id: string): Promise<Permission> {
    ...
    return await this.permissionModel.findByIdAndUpdate(id, { estado: false }, { returnDocument: 'after' })
      .select('-__v')
      .exec() as Permission;
  }
  ...
  async findAllInactive(): Promise<Permission[]> {
    return await this.permissionModel.find({ estado: false })
      .select('-__v')
      .populate('modulo', 'nombre _id')
      .exec();
  }
  ...
  async restore(id: string): Promise<Permission> {
    ...
    if (!permission) throw new NotFoundException(`Permiso no localizado para restauración`);
    if (permission.estado) throw new ConflictException(`El permiso ya se encuentra en estado activo`);
  ...
  async findOneInactive(id: string): Promise<Permission> {
    ...
    if (permission.estado) throw new ConflictException(`El permiso consultado está activo`);
  ...
  async findOnemodule(id: string): Promise<Module> {
    ...
    if (!module) throw new NotFoundException(`Módulo de referencia no encontrado`);
    if (!module.estado) throw new ForbiddenException(`Operación denegada: El módulo asociado está inactivo`);
  ...
  private extractModuleId(permission: Permission): string {
    ...
    throw new NotFoundException('No se pudo determinar el módulo asociado al permiso');
  }
