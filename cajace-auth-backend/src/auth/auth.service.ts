...
export class AuthService {
  ...
  async login(loginDto: LoginDto, req: Request, res: Response) {
    ...
    if (!isPasswordValid) {
      throw new UnauthorizedException('Credenciales de acceso inválidas: Contraseña incorrecta');
    }
  ...
  async refreshFromGuard(refreshToken: string, req: Request, res: Response): Promise<any> {
    ...
    if (lockedSession) {
      ...
      if (!updatedSession?.estado) throw new UnauthorizedException('Sesión invalidada durante el proceso');
      return payload; 
    }
    ...
    if (!session) throw new UnauthorizedException('Sesión no disponible o bloqueada por seguridad');
    ...
    try {
      ...
      if (!user) throw new UnauthorizedException('Acceso denegado: Usuario inactivo o inexistente');
  ...
  async allSessions(): Promise<Session[]> {
    return await this.sessionModel.find({ estado: true })
      .select('-refreshToken -__v')
      .populate({ 
        path: 'usuario', 
  ...
  async revokeSession(
    ...
    if (!session || !session.estado) throw new NotFoundException('La sesión solicitada no existe o ya está inactiva');

    if (!esAdmin && session.usuario.toString() !== usuarioId) {
      throw new ForbiddenException('Acceso denegado: No posee privilegios para revocar esta sesión');
    }
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    const user = await this.userModel.findOne({ email: dto.email, estado: true }).exec();

    if (!user) {
      // RESPUESTA GENÉRICA PARA EVITAR ENUMERACIÓN DE USUARIOS (SEGURIDAD)
      return { message: 'Si el correo existe, recibirá un código de verificación en breve' };
    }
  ...
  async verifyCode(dto: VerifyCodeDto) {
    ...
    if (!user) throw new UnauthorizedException('Error de validación: Datos de recuperación incorrectos');
    ...
    if (!reset) throw new UnauthorizedException('El código ingresado es inválido o ya fue utilizado');
    if (reset.expiraEn < new Date()) throw new UnauthorizedException('El código de verificación ha expirado');
  ...
  async resetPassword(dto: ResetPasswordDto) {
    const user = await this.userModel.findOne({ email: dto.email, estado: true }).exec();
    if (!user) throw new UnauthorizedException('Operación no permitida: Usuario inválido');
    ...
    if (!reset || reset.expiraEn < new Date()) {
      throw new UnauthorizedException('Error de seguridad: El código es inválido o ha expirado');
    }
    ...
    await this.notificationsService.notificarCambioPassword(user._id.toString(), user.nombre);

    return { message: 'La contraseña ha sido restablecida y las sesiones han sido cerradas por seguridad' };
  }
  ...
  async buildRequestUserContext(usuarioId: string, sessionId: string) {
    ...
    const permisosRol = (user.rol as any)?.permisos?.map((p: any) => p.nombre) ?? [];
  ...
  private async getUserWithPermisos(userId: string): Promise<User> {
    return await this.userModel.findOne({ _id: userId, estado: true })
      .select('+password')
      .populate({
        path: 'rol',
        populate: {
  ...
  async findOneByEmail(email: string): Promise<User> {
    ...
    if (!user) throw new UnauthorizedException('Acceso denegado: El correo electrónico no está registrado');
    if (!user.estado) throw new UnauthorizedException('Acceso denegado: La cuenta de usuario se encuentra inactiva');
  ...
  private formatearUsuario(user: any) {
    ...
    return {
      ...
      email: user.email,
      rol: user.rol?.nombre,
      permisos: agruparPermisos((user.rol as any)?.permisos ?? []),
      estado: user.estado,
