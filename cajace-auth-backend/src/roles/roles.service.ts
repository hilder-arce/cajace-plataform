...
export class RolesService {
  ...
  async create(createRoleDto: CreateRoleDto, creadoPor: string): Promise<Role> {
    const existingRole = await this.roleModel.findOne({ nombre: createRoleDto.nombre });
    if (existingRole) {
      throw new ConflictException(`Conflicto: El rol con el nombre "${createRoleDto.nombre}" ya existe`);
    }
    ...
    await this.notificationsService.notificarNuevoRol(createRoleDto.nombre, creadoPor);
  ...
  async findAll(): Promise<Role[]> {
    return await this.roleModel.find({ estado: true })
      .select('-__v')
      .populate({
        path: 'permisos',
        select: 'nombre _id',
        populate: { path: 'modulo', select: 'nombre _id' },
      })
      .exec();
  }
  ...
  async findOne(id: string): Promise<Role> {
    ...
    if (!role.estado) throw new NotFoundException('El rol solicitado se encuentra inactivo');
  ...
  // ======================================================
  async update(id: string, updateRoleDto: UpdateRoleDto, actualizadoPor = 'Sistema'): Promise<Role> {
    const currentRole = await this.findOne(id);
    const currentPermissionMap = this.buildPermissionMap(currentRole.permisos as RolePermissionRef[]);
    ...
    if ((permisos && permisos.length > 0) || (permisosEliminar && permisosEliminar.length > 0)) {
    ...
    await this.roleModel.findByIdAndUpdate(id, updateData, { returnDocument: 'after' })
      .select('-__v')
      .exec();

    const updatedRole = await this.findOne(id);
    const updatedPermissionMap = this.buildPermissionMap(updatedRole.permisos as RolePermissionRef[]);
  ...
  async findAllInactive(): Promise<Role[]> {
    return await this.roleModel.find({ estado: false })
      .select('-__v')
      .populate({
        path: 'permisos',
        select: 'nombre _id',
  ...
  async restore(id: string): Promise<Role> {
    ...
    return await this.roleModel.findByIdAndUpdate(id, { estado: true }, { returnDocument: 'after' })
      .select('-__v')
      .exec() as Role;
  }
  ...
  async findOneInactive(id: string): Promise<Role> {
    ...
    if (role.estado) throw new ConflictException('El rol consultado ya esta activo');
  ...
  async findOnePermission(id: string): Promise<Permission> {
    ...
    if (!permission) throw new NotFoundException('Permiso de referencia no encontrado');
    if (!permission.estado) throw new ConflictException('Operacion denegada: El permiso asociado esta inactivo');

    return permission;
  }

  private buildPermissionMap(permissions: RolePermissionRef[]): Map<string, string> {
    return new Map(
      permissions
        .map((permission) => {
