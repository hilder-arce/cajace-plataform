        ...
        if (accessToken) {
            try {
                ...
                if (payload.sessionId) {
                    const sessionValida = await this.authService.validateSession(payload.sessionId);
                    if (!sessionValida) {
                ...
                req['user'] = payload.sessionId
                    ? await this.authService.buildRequestUserContext(payload.sub, payload.sessionId)
                    : payload;
