...
import { CajaceLogoComponent } from '../../../shared/components/cajace-logo/cajace-logo.component';
...
const SHELL_NAV_METADATA: Record<string, { label: string; icon: string; exact?: boolean }> = {
  ...
  modules: { label: 'Modulos', icon: 'view_module' },
  ...
  private handleRolePermissionsUpdated(event: RolePermissionsUpdatedEvent): void {
    this.authService.me().subscribe({
      next: () => {
        ...
        this.alertService.show(
          `Un administrador actualizó los permisos del rol ${event.roleName}. El panel lateral fue actualizado y serás llevado al dashboard.`,
          'success',
          4200,
        );
